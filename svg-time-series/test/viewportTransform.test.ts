...
import type { ViewportTransform as ViewportTransformClass } from "../src/ViewportTransform.ts";
import { scalesToDomMatrix } from "../src/utils/domMatrix.ts";
...
beforeAll(async () => {
  ({ ViewportTransform } = await import("../src/ViewportTransform.ts"));
});
...
describe("ViewportTransform", () => {
  ...
  it("round-trips between screen and model coordinates", () => {
    ...
    const [rx1, rx2] = roundTripX;
    expect(rx1).toBeCloseTo(20);
    expect(rx2).toBeCloseTo(40);
    ...
    const [ry1, ry2] = roundTripY;
    expect(ry1).toBeCloseTo(20);
    expect(ry2).toBeCloseTo(40);
  });
